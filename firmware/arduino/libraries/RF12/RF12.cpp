...
// 
// PORTED VERSION TO USE WITH ATMEGA1284P & DUINOS
// BY LE DANG DUNG
// Note : Also added RSSI reading
// 
#include "RF12.h"
#include <avr/io.h>
...
uint8_t rf12_recvDone () {
    if (rxstate == TXRECV && (rxfill >= rf12_len + 5 || rxfill >= RF_MAX)) {
        ...
        if (!(rf12_hdr & RF12_HDR_DST) ||
            ...
            count = 0; // DDL: reset our counter for later ADC    
...
void rf12_initialize (uint8_t id, uint8_t band, uint8_t g) {
    ...
    rf12_xfer(0xA4B0); // 433MHz
    //rf12_xfer(0xC606); // approx 49.2 Kbps, i.e. 10000/29/(1+6) Kbps
    rf12_xfer(0xC610); // approx 20.284 Kbps, i.e. 10000/29/(1+r) Kbps
    //rf12_xfer(0x94A2); // VDI,FAST,134kHz,0dBm,-91dBm 
    rf12_xfer(0x9460); // RX: VDI,FAST,270kHz,max gain,-103dBm 
    ...
    rf12_xfer(0x9890); // !mp,150kHz,MAX OUT 
...
// DDL : added functions
// This will start the ADC to convert Analog RSSI, called whenever 
// data arrives (see rf12_interrupt())
// note: when ADC done, it will interrupt the main program
// so a callback IRQ is required to catch this event.
void startADC(void) 
{ 
...
// use this in the callback to stop ADC
void stopADC(void)
{   
    ADCSRA &= ~(1 << ADEN);    
}